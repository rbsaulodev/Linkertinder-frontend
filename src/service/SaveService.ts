...
export function saveCandidato(
  ...
  if (!validateNome(nome))          errors.push('Nome completo inválido. Informe nome e sobrenome.');
  if (!validateEmail(email))        errors.push('E-mail inválido. Ex: joao@email.com');
  if (!validateCPF(cpf))           errors.push('CPF inválido. Ex: 123.456.789-09');
  if (!validatePhone(numbero))      errors.push('Telefone inválido. Ex: (11) 91234-5678');
  if (!validateLinkedIn(linkedin))  errors.push('LinkedIn inválido. Ex: https://linkedin.com/in/usuario');
  if (!validateIdade(idadeStr))     errors.push('Idade inválida. Deve ser entre 16 e 80 anos.');
...
export function saveEmpresa(
  ...
  if (!validateNomeEmpresa(nome))   errors.push('Nome da empresa inválido.');
  if (!validateEmail(email))        errors.push('E-mail inválido. Ex: rh@empresa.com');
  if (!validateCNPJ(cnpj))         errors.push('CNPJ inválido. Ex: 11.222.333/0001-81');
  if (!validateCEP(cep))           errors.push('CEP inválido. Ex: 01310-100');
  if (pais && !validatePais(pais))  errors.push('País inválido. Use apenas letras. Ex: Brasil');
  if (estado && !validateEstado(estado)) errors.push('Estado inválido. Use a sigla de 2 letras. Ex: SP');
  if (descricao && !validateDescricao(descricao)) errors.push('Descrição muito curta. Mínimo 10 caracteres.');
